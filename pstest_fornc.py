...
def ncmod(ncfile:str):
    global file,vkey,var,dkey,dim,plot_sect
    ...
    def plot_sect(data:vkey,bbox=[-179.5, -89.5, 179.5, 89.5],):
        i = len(vkey)
        np.arange(bbox[0],bbox[2]+1,1)
        #cot=[x for x in range(1,i+1)]
        #ncot = [str(x)+'.'+y for x,y in zip(cot,vkey)];print(ncot)
        #data:np.array ,bbox=[-179.875, -89.875, 179.875, 89.875],level=None):
        #print(ncot)
        #input('select var:')
        pass
